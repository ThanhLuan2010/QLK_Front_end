...
import { createDebtor, Get_all_DEBTOR } from "../xuatkho/handleCreateDebtor";
import { Update_DOANHTHU_BY_storeID_thoidiem } from "../xuatkho/handleCreateDebtor";
import i18n from "../../i18n/i18n";
import { useTranslation } from "react-i18next";
import { EditProduct } from "../contacts/handleproduct";
import { DeletePhieuOrder } from "../xuatkho/handlePhieustore";
...
const PendingOrders = () => {
            ...
            <table className="custom-table">
              <thead>
                <tr>
                  ...
                  <th style={{ color: "red" }}>{i18n.t("REMAIN")}</th>
                </tr>
              </thead>
              <tbody>
                {content.map((item) => {
                  const stockProduct = stateProductfetch.find(
                  ...
                  return (
                    <tr key={item.id}>
                      ...
                      <td>{item.soluong}</td>
                      <td style={{ color: "red" }}>
                        {stockProduct ? stockProduct.soluong : 0}
                      </td>
                    </tr>
                  );
                })}
  ...
  const columns = [
    { field: "id", flex: 0.75, headerName: `${i18n.t("MAPX_PX")}` },
    {
      field: "phieustoreID",
  ...
  const getCurrentStock = async (storeID) => {
    try {
      const response = await Get_all_Product_By_StoreID(storeID);
  ...
  const updateEditproduct = async (x) => {
    for (const item of x) {
  ...
  const createBill = async () => {
    ...
    const currentStock = await getCurrentStock(statechinhanh);
    let insufficientStockProducts = [];
    selectedRow[0].arrayProduct.forEach((product) => {
      const stockProduct = currentStock.find((item) => item.id === product.id);
      if (stockProduct && stockProduct.soluong < product.soluong) {
        insufficientStockProducts.push(product);
      }
    });

    // Show warning if any product has insufficient stock
    if (insufficientStockProducts.length > 0) {
      alert("Số lượng sản phẩm trong kho không đủ. Vui lòng kiểm tra lại.");
      setisloading(false);
    ...
    let lenghtState = maxNumber + 1;
    console.log(lenghtState);
    // Lấy thông tin từ selectedRow
    const selectedRowData = selectedRow[0];
    const userID = selectedRowData.userID;
    const noiban = selectedRowData.storeID;
    const OrderID = selectedRowData.id;
    const fetchPhieuStoreData = await Get_all_Phieu_Store();
    const phieuStoreData = JSON.parse(fetchPhieuStoreData);
    const noimuaData = phieuStoreData.find(
      (item) => item.id === selectedRowData.phieustoreID
    );

    const noimua = noimuaData ? noimuaData.StoreID : "";

    // Tính giamua và giaban
    let giamua = 0;
    selectedRowData.arrayProduct.forEach((element) => {
      giamua += parseFloat(element.sotien) * parseFloat(element.soluong);
    });
    // const giaban = giamua + giamua * 0.15;
    const giaban = giamua;
    giamua = giaban - giaban * 0.15;
    ...
    console.log("addformbill", addformbill);
    ...
    try {
      ...
      if (JSON.parse(check).success) {
        ...
        const checktemp = await Get_all_Product_By_StoreID(addformbill.noimua);
        let lenghtState = 0;
          ...
          selectedRow.forEach(async (obj) => {
            ...
            updateMoney.forEach(async (obj) => {
              ...
              const createProductPromises = a.map(async (obj, index) => {
                ...
                const filteredArray = JSON.parse(resolvedResult).find(
                  (i) => i.id === obj.id
                );
                if (filteredArray) {
                  ...
                  await EditProduct({
                    ...filteredArray,
                    soluong: filteredArray.soluong + obj.soluong,
          ...
          selectedRow.forEach(async (obj) => {
            ...
            updateMoney.forEach(async (obj) => {
              ...
              const createProductPromises = a.map(async (obj, index) => {
                ...
                const filteredArray = JSON.parse(checktemp).find(
                  (i) => i.id === obj.id
                );
                if (filteredArray) {
                  console.log("filteredArray", filteredArray);
        ...
        await updateEditproduct(selectedRow[0].arrayProduct);
        setisloading(false);
        await fetchingOrderBy_storeID_By_year_month(
          statechinhanh,
          formattedDate
    ...
    if (check instanceof Promise) {
      ...
      const FormcreateDebTor = {
        id:
          "DEB" +
          "-" +
          selectedRow[0].phieustoreID.split("-")[1] +
          "-" +
          lenghtState,
        Debtor_BranchID: addformbill.noimua,
        Owner_BranchID: addformbill.noiban,
        sotienNo: addformbill.giaban,
  ...
  const fetchingOrderBy_storeID_By_year_month = async (storeID, thoidiem) => {
    ...
    try {
      ...
      if (check instanceof Promise) {
        ...
        const nonDeletedOrders = orders.filter(
          (order) => !order.status.includes("DELETED")
        );
  ...
  return (
    <Box m="20px">
            ...
            <div class="modal-footer">
              ...
              >
                {isloading ? (
                  ...
                  <Typography> {i18n.t("BTN_XACNHAN")}</Typography>
                )}
              </button>
            </div>
          </div>
      ...
      <Button
        ...
        disabled={selectionModel.length !== 1 || isloading}
      >
        {isloading ? (
          <CircularProgress size={24} color="inherit" />
        ) : (
          <Typography> {i18n.t("BTN_XACNHANYEUCAU_NP")}</Typography>
        )}
      </Button>
      <Box className={classes.datagrid} height="65vh">
        <DataGrid
          editMode="row"
          checkboxSelection
